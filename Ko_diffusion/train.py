...
use_amp = True
resume_train = False
file_num = "not_subset_cond_emb_lr_1e-4"
...
if __name__ == '__main__':
    ...
    #Set save file
    result_image_path = os.path.join("results", 'font_{}'.format(file_num))
    result_model_path = os.path.join("models", 'font_{}'.format(file_num))
    ...
    if resume_train:
        #load weight
        model.load_state_dict(torch.load(f'./models/font_{file_num}/ckpt_2_{start_epoch}.pt'))
        ema_model.load_state_dict(torch.load(f'./models/font_{file_num}/ema_ckpt_2_{start_epoch}.pt'))

        #load optimzer
        optimizer.load_state_dict(torch.load(f'./models/font_{file_num}/optim_2_{start_epoch}.pt'))
    ...
    for epoch_id in range(start_epoch, n_epochs+1):
        ...
        if cond_mode == 3 and epoch_id > change_cond_epoch:
            cond_mode = 1
        ...
        for i, (image, content) in enumerate(pbar):
            ...
            if i % (len_dataloder//10) == 0:
                wandb.log({"train_mse_loss": loss}, commit=True)

        toc = time()
        wandb.log({'train_time':(toc-tic)//60}, commit=True)
