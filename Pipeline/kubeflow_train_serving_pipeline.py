...
@partial(create_component_from_func,
        ...
        packages_to_install=['urllib3==1.26.14'])
def load_fonts(style_url:str,sytle_path:str,fonts_url:str, font_path:str):
    import tarfile
    import os
...
@partial(create_component_from_func,
        base_image='python:3.10',
        packages_to_install=['pandas==2.0.0','pillow==9.4.0'])
def make_image_csv(image_size : int, train_string : str, fonts_base_path : str, result_path : str, csv_path : str):
    import os
    import pandas as pd
    ...
    hangul_codePoint = [format(ord(ch),'X') for ch in train_string]
    ...
    for uni in hangul_codePoint:
        ...
        for fonts_folder in fonts_folder_list:
                ...
                for ttf in fonts:
                    ...
                    draw_image.text(((image_size-x)/2, (image_size-y)/2), unicodeChars, font=font, fill='black')

                    # Set file name
                    # file_name = os.path.join(result_path, ttf[:-4]+"_"+unicodeChars)
                    ttf_name = ttf[:-4].replace(" ","")
                    # print(ttf[:-4],ttf_name)
                    file_name = result_path + "/" + ttf_name + "_" + unicodeChars + ".png"
...
@partial(create_component_from_func,
        base_image='python:3.10',
        packages_to_install=['tqdm==4.65.0','pandas==2.0.0',' pillow==9.4.0','torch==2.0.0+cu118','torchvision==0.15.1+cu118'])
def model_training(gpu_num:int, batch_size: int, input_size : int, n_epochs : int, num_classes : int, mode : int, lr : float, result_model_path : str, style_path: str):
    import os
    import tqdm
    import math
    import random
...
def my_pipeline(style_url:str, fonts_url:str,image_size: int, train_string:str, fonts_base_path:str,result_path:str,csv_path:str,gpu_num:int, batch_size: int, n_epochs : int, num_classes : int, mode : int, lr : float, result_model_path : str, style_path: str):
    font = load_fonts(style_url,style_path,fonts_url,fonts_base_path)
    font.apply(onprem.mount_pvc(pvc_name="diffusion", volume_name="test-lee", volume_mount_path="pvc"))
    
    data = make_image_csv(image_size,train_string,fonts_base_path,result_path,csv_path)
    data.apply(onprem.mount_pvc(pvc_name="diffusion", volume_name="test-lee", volume_mount_path="pvc"))
    data.after(font)
    
    model = model_training(gpu_num, batch_size, image_size, n_epochs, num_classes, mode, lr, result_model_path, style_path)
    model.apply(onprem.mount_pvc(pvc_name="diffusion", volume_name="test-lee", volume_mount_path="pvc"))
    model.set_cpu_limit(cpu="8").set_memory_limit(memory="16G")
