...
def make_dataset_csv(csv_path : str, file_folder_path : str):
    import os
    import pandas as pd
...
@partial(create_component_from_func,
        base_image='python:3.10',
        packages_to_install=['tqdm==4.65.0','pandas==2.0.0',' pillow==9.4.0','torch==2.0.0+cu118','torchvision==0.15.1+cu118'])
def model_training(gpu_num:int, batch_size: int, input_size : int, n_epochs : int, num_classes : int, mode : int, lr : float, result_model_path : str, style_path: str) -> float:
    import os
    import tqdm
    import math
    import random
...
@partial(create_component_from_func,
        base_image='python:3.10',
        packages_to_install=[])
def model_evalute(model_value: float,model_evalute_csv_path :str,model_path)->bool:
    import pandas as pd
...
def my_pipeline(image_size: int,result_path:str,csv_path:str,gpu_num:int, batch_size: int, n_epochs : int, num_classes : int, mode : int, lr : float, result_model_path : str, style_path: str):
    
    data = make_dataset_csv(csv_path=csv_path, file_folder_path=result_path)
    data.apply(onprem.mount_pvc(pvc_name="diffusion", volume_name="test-lee", volume_mount_path="pvc"))
    data.set_display_name("Make a train.csv file")
    ...
    model.after(data)
    flag = model_evalute(model.output,"pvc/model.csv",result_model_path)
    print(flag.output)
    with Condition(flag.output == True,"Best-Model"):
        handle = make_handle()
    ...
    with Condition(flag.output == False,name="Not-a-Best-Model"):
        end = continous_learning_end()
        end.after(flag)
    
if __name__ == "__main__": 
    compiler.Compiler().compile(my_pipeline, "KD_Font_kubeflow_continuous_training.yaml")
